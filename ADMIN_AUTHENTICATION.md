...
```bash
curl -X POST http://192.168.1.13:8000/api/auth/login \
...
```bash
curl -X GET http://192.168.1.13:8000/api/admin/organizer-requests \
...
```bash
curl -X POST http://192.168.1.13:8000/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{
